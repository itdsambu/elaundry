...
class Penerimaan_laundry extends CI_Controller
{
    ...
    function ajaxGetDataTK()
    {
        ...
        $data['get_dataTK']   = $dataTK;

        $this->load->view('tbl_trnLaundry/ajaxDetailTK', array_merge($data));
    }
    ...
    public function unique_nota_by_pos($noNota)
    {
        $pos_ldy = $this->input->post('pos_ldy');

        $exists = $this->db->where('NoNota', $noNota)->where('pos_ldy', $pos_ldy)->get('tblLaundry_TrnLayanan')->num_rows();

        if ($exists > 0) {
            $this->form_validation->set_message('unique_nota_by_pos', '%s sudah ada untuk lokasi tersebut. Silahkan pilih kembali.');
            return FALSE;
        }
    ...
    function simpanData()
    {
        $this->form_validation->set_rules(
            'NoNota',
            'Nomor Nota',
            'required|callback_unique_nota_by_pos',
            array(
