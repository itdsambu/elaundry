...
class Approval_laundry extends CI_Controller
{
    ...
    function ajaxData2()
	{
        ...
        switch ($bulan) {
            ...
            case 8: $bln = '08'; break;
            case 9: $bln = '09'; break;
            default: $bln = str_pad($bulan, 2, '0', STR_PAD_LEFT);
        }
        
        $blnGanda2 = str_pad(($bulan == 12) ? '01' : ($bulan + 1), 2, '0', STR_PAD_LEFT);
        $blnKar = str_pad($blnKar, 2, '0', STR_PAD_LEFT);
        ...
        if ($periode == 'p1') {
            ...
            $jarak1 = "{$tahun}-{$bln}-16";
            $jarak2 = ($bulan == 12) ? (intval($tahun) + 1) . "-01-01" : "{$tahun}-{$blnGanda2}-01";
        } else if ($periode == 'bulanan') {
            $jarak1_tahun = ($bulan == 1) ? intval($tahun) - 1 : $tahun; // Jika bulan Januari, gunakan tahun sebelumnya
			$jarak1 = "{$jarak1_tahun}-{$blnKar}-26";
			$jarak2 = ($bulan == 12) ? (intval($tahun) + 1) . "-01-25" : "{$tahun}-{$bln}-25";
	...
	function approveTrnLaundry()
	{
		...
		$getPersonid  =  $this->m_penerimaan_laundry->get_getPersonId($signid, $user_name);

		foreach ($getPersonid->result_array() as $item) {
			$personalid        = $item['personalid'];
			$personalstatus    = $item['personalstatus'];
		}

		// $angkaBulan = ($bln < 10) ? '0'.$bln : $bln;
