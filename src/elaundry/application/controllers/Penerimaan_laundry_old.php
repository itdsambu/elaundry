...
class Penerimaan_laundry extends CI_Controller
{
	...
	function tambahData()
	{
		...
		// 	$new_no = (int)$no_nota + 1;
		// }
		// $data['no_nota'] = '00' . $new_no . '/LDY/' . date('m') . '/' . date('Y');
		// $data['no_nota'] = $new_no;
	...
	function dt_nota()
	{
		...
		// $tanggal_selesai = date("Y-m-d", strtotime($this->input->post('TanggalTerima')));
		$tahun           = $this->input->post('tahun');
		$bulan           = $this->input->post('bulan');
		$pos_ldy         = $this->input->post('pos_ldy');

		// $cek_data_rekap = $this->M_laundry->cek_data_rekap2($tanggal_selesai, $StatusTK);

		// if ($cek_data_rekap->num_rows() > 0) {
		// 	echo $data = "100";
		// } else {
		// 	$result = $this->M_laundry->dt_nota($tahun, $bulan, $pos_ldy);
		// 	$data = '';
