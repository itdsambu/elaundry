...
class PDF extends FPDF {
        function Header(){
          ...
          $this->SetX(18);
          $this->Image(base_url().'assets/layouts/layout3/img/logopt.png',17,12,23,20);
          ...
          $this->Cell(10,5,'Bulan',0,0,'L');
          $this->Cell(5,5,':',0,0,'L');
          $this->SetXY(170,$this->GetY());
          $this->MultiCell(30,5,$this->bulan.' - '.$this->tahun,0,'L');
          ...
          $this->SetX(35);
          $this->Cell(90,5,'Nama Suplier','LTB',0,'C');
          $this->SetX(125);
          $this->Cell(75,5,'Total','LTBR',0,'C');
          $this->Ln(5);
    ...
    $yawal = $pdf->GetY();
      foreach($getData as $get){
        ...
        $pdf->SetXY(35,$y);
        $pdf->MultiCell(90,5,$get->nama_suplier,0,'L');
        $pdf->SetXY(130,$y);
        $pdf->MultiCell(110,5,number_format($get->Total),0,'C');
        $pdf->Ln();
      ...
      foreach($getDataTotal as $gt){   
        ...
        $pdf->cell(110,5,'Grand Total','LB',0,'C');
        $pdf->SetXY(125,$pdf->GetY());
        $pdf->cell(75,5,number_format($gt->GrandTotal,2),'LBR',0,'R');
        $pdf->Ln(5);
    }
    $pdf->SetFont('Times','B',9);
    foreach($getApprove as $app) {
        if($app->Approve_kabag == 1) {
            ...
            $pdf->MultiCell(195,5,'Tanggal Efektif : '.date('d-m-Y',strtotime($app->Approve_kabagDate)),0,'L');
        }else{
            $pdf->SetXY(20,$pdf->GetY()+5);
